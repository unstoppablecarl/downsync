import fs from 'fs'
import { renderTemplateVariables, stripQuickReferenceContentTags } from '../../../markdown/support/markdown-helpers.js'
import { RULES_LAST_UPDATED, RULES_VERSION } from '../../../versioning.js'

let path = 'src/markdown/downsync-rules.md'

export const markdown = await fs.promises.readFile(path, 'utf-8')
    .then(markdown => stripQuickReferenceContentTags(markdown))
    .then(markdown => renderTemplateVariables(markdown, {
        RULES_VERSION,
        RULES_LAST_UPDATED,
    }))
